	...
	class phpFlickpress {
		...
		function cache ($request, $response)
		{
			...
			if ($this->cache == 'db') {
				...
				$result = mysql_fetch_row($result);
				if ( $result[0] ) {
					$sql = "UPDATE " . $this->cache_table . " SET response = '" . mysql_real_escape_string($response) . "', expiration = '" . strftime("%Y-%m-%d %H:%M:%S") . "' WHERE request = '" . $reqhash . "'";
					mysql_query($sql, $this->cache_db);
				} else {
					$sql = "INSERT INTO " . $this->cache_table . " (request, response, expiration) VALUES ('$reqhash', '" . mysql_real_escape_string($response) . "', '" . strftime("%Y-%m-%d %H:%M:%S") . "')";
					mysql_query($sql, $this->cache_db);
				}
			} elseif ($this->cache == "fs") {
				$file = $this->cache_dir . "/" . $reqhash . ".cache";
		...
		function auth ($perms = "read", $remember_uri = true) {
			...
			if (empty($_SESSION['phpFlickpress_auth_token']) && empty($this->token)) {
				if ( $remember_uri === true ) {
					session_register('phpFlickpress_auth_redirect');
					$_SESSION['phpFlickpress_auth_redirect'] = $_SERVER['REQUEST_URI'];
				} elseif ( $remember_uri !== false ) {
					session_register('phpFlickpress_auth_redirect');
					$_SESSION['phpFlickpress_auth_redirect'] = $remember_uri;
				}
...
if ( !class_exists('phpFlickpress_pager') ) {
	class phpFlickpress_pager {
		var $phpFlickpress, $per_page, $method, $args, $results, $global_phpFlickpress;
		var $total = null, $page = 0, $pages = null, $photos, $_extra = null;
		
		
		function phpFlickpress_pager($phpFlickpress, $method = null, $args = null, $per_page = 30) {
			$this->per_page = $per_page;
			$this->method = $method;
			$this->args = $args;
			$this->set_phpFlickpress($phpFlickpress);
		}
		
		function set_phpFlickpress($phpFlickpress) {
			if ( is_a($phpFlickpress, 'phpFlickpress') ) {
				$this->phpFlickpress = $phpFlickpress;
				if ( $this->phpFlickpress->cache ) {
		...
		function load($page) {
			...
			if ( $this->phpFlickpress->cache ) {
				...
				if ( floor($min/500) == floor($max/500) ) {
					...
					$this->results = $this->phpFlickpress->call($this->method, $this->args);
					if ( $this->results ) {
